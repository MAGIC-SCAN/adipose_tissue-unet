...
# Test subdirectories (where organized test files are located)
TEST_PSEUDO_DIR = PSEUDO_DIR / "test"
TEST_MASKS_DIRS = {
    "bubbles": JSON_MASKS_DIR / "bubbles" / "test",
    "fat": JSON_MASKS_DIR / "fat" / "test", 
    "muscle": JSON_MASKS_DIR / "muscle" / "test"
}
...
DEFAULTS = {
    ...
    "include_test_set": True,  # Include test set from test subdirectories
...
def create_build_log(args, build_root: Path, stage: str, **kwargs):
    ...
    log_data = {
        ...
        "data_paths": {
            ...
            "test_masks_dirs": {k: str(v) for k, v in TEST_MASKS_DIRS.items()},
    ...
    with open(summary_file, 'w', encoding='utf-8') as f:
        ...
        for cls, path in paths_info['test_masks_dirs'].items():
            f.write(f"Test {cls.capitalize()} Masks: {path}\n")
        f.write("\n")
...
def find_pseudo_image(image_stem: str) -> Path | None:
    """Resolve the pseudocolor JPG given an image stem (main directory)."""
    p = PSEUDO_DIR / f"{image_stem}.jpg"
    return p if p.exists() else None

def find_pseudo_image_in_test(image_stem: str) -> Path | None:
    """Resolve the pseudocolor JPG in test directory given an image stem."""
    p = TEST_PSEUDO_DIR / f"{image_stem}.jpg"
    return p if p.exists() else None
...
# ------------------------------
def collect_target_bases(target_cls: str) -> Set[str]:
    """Collect training/validation image stems from main directories."""
...
def collect_test_bases(target_cls: str) -> Set[str]:
    """Collect test image stems from test subdirectories."""
    if not TEST_PSEUDO_DIR.exists():
        print(f"[Test] No test directory found: {TEST_PSEUDO_DIR}")
        return set()
    
    # Get image stems from test images directory
    test_image_stems = set()
    for jpg_file in TEST_PSEUDO_DIR.glob("*.jpg"):
        test_image_stems.add(jpg_file.stem)
    
    # Verify test JSON annotations exist
    test_json_dir = TEST_MASKS_DIRS.get(target_cls)
    if not test_json_dir or not test_json_dir.exists():
        print(f"[Test] No test JSON directory for '{target_cls}': {test_json_dir}")
        return set()
    
    test_bases = set()
    for j in sorted(test_json_dir.glob("*.json")):
        base = parse_image_stem_from_json(j, target_cls)
        if base in test_image_stems:
            test_bases.add(base)
    
    print(f"[Test] Found {len(test_bases)} test slide(s) with target '{target_cls}' JSONs.")
    return test_bases
...
def build_masks_from_json(args, training_bases: Set[str], test_bases: Set[str]):
    ...
    # Process test data (test directories) if enabled
    if getattr(args, 'include_test_set', True) and test_bases:
        ...
        all_work_items.extend(_prepare_mask_work_items(
            ...
            lambda cls: TEST_MASKS_DIRS.get(cls),
            args, masks_out_root, overlays_root, min_confidence, size_cache
        ))
...
def tile_coords(h: int, w: int, tile: int, stride: int):
    ...
    # Calculate steps more carefully to avoid out-of-bounds
    if stride >= tile:
        # Non-overlapping or gap case - simpler calculation
        x_steps = w // tile
        y_steps = h // tile
    else:
        # Overlapping case - ensure we don't go out of bounds
...
def split_dataset(args, training_bases: Set[str], test_bases: Set[str]):
    ...
    # All test tiles go to test directory
    tst = test_tiles
    
    # Split training tiles into train and validation only
    if args.split_by_slide:
        ...
        # Split training slides into train and val
        n_val = max(1, int(len(slides) * args.val_ratio))
        val_slides = set(slides[:n_val])
        
        # Assign training tiles to train/val splits
        trn, val = [], []
        ...
        n_val = max(1, int(len(training_tiles) * args.val_ratio))
        val = training_tiles[:n_val]
        trn = training_tiles[n_val:]

    # Copy files to respective directories
    print(f"[Split] INTEGRATED MODE: train={len(trn)}  val={len(val)}  test={len(tst)}  "
          f"(test from separate directory, val_ratio={args.val_ratio})")
    _copy(trn, "train")
    _copy(val, "val")
    _copy(tst, "test")
...
def parse_args():
    ...
    p.add_argument("--data-root", type=str, default=str(DATA_ROOT), 
                   help="Project data root (contains Pseudocolored/ and Masks/)")
    ...
    p.add_argument("--include-test-set", dest="include_test_set", action="store_true",
                   ...
                   help="Include test set from test subdirectories")
...
def main():
    ...
    global DATA_ROOT, PSEUDO_DIR, JSON_MASKS_DIR, BUILD_ROOT, TEST_PSEUDO_DIR, TEST_MASKS_DIRS
    DATA_ROOT = Path(args.data_root)
    PSEUDO_DIR = DATA_ROOT / "Pseudocolored"
    JSON_MASKS_DIR = DATA_ROOT / "Masks"
    
    # Update test subdirectories
    TEST_PSEUDO_DIR = PSEUDO_DIR / "test"
    TEST_MASKS_DIRS = {
        "bubbles": JSON_MASKS_DIR / "bubbles" / "test",
        "fat": JSON_MASKS_DIR / "fat" / "test", 
    ...
    # Create timestamped build directory to avoid overwriting existing datasets
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    BUILD_ROOT = DATA_ROOT / f"_build_{timestamp}"
    ...
    training_bases = collect_target_bases(args.target_mask)
