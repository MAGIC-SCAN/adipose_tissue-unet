...
class CheckpointMetadata:
    ...
    timestamp: str
    suffix: str  # Model variant suffix (e.g., 'adamw', 'perc', 'tta_like_aug', or '')
    ...
    def get_label(self) -> str:
        ...
        # Line 1: Timestamp and suffix (if present)
        if self.suffix:
            lines = [f"{self.timestamp} ({self.suffix})"]
        else:
...
class CheckpointMetricsExtractor:
    ...
    def __init__(self, checkpoints_dir: str = "../checkpoints/segmentation",
    ...
    def discover_checkpoints(self, checkpoint_names: Optional[List[str]] = None) -> List[Path]:
        ...
        Args:
            checkpoint_names: Optional list of specific checkpoint names/timestamps.
                             If a timestamp is provided (e.g., "20251102_015405"), all checkpoints
                             with that timestamp will be matched regardless of suffix.
        ...
        if checkpoint_names:
            ...
            for name in checkpoint_names:
                ...
                if checkpoint_path.exists() and checkpoint_path.is_dir():
                    ...
                    print(f"  ✓ Exact match: {checkpoint_path.name}")
                    continue
                
                # Check if name looks like a timestamp (YYYYMMDD_HHMMSS)
                is_timestamp = re.match(r'^\d{8}_\d{6}$', name)
                
                if is_timestamp:
                    # Find all checkpoints with this timestamp
                    pattern = f"{name}_*"
                    ...
                    if matches:
                        ...
                        print(f"  ✓ Found {len(matches)} checkpoint(s) with timestamp {name}:")
                        for m in matches:
                            print(f"    - {m.name}")
                    else:
                        print(f"  ⚠️  No checkpoints found with timestamp: {name}")
                else:
                    # Not a timestamp - try pattern matching
                    matches = list(self.checkpoints_dir.glob(f"*{name}*"))
                    matches = [m for m in matches if m.is_dir()]
                    
                    if matches:
                        checkpoints.extend(matches)
                        print(f"  ✓ Pattern match found {len(matches)} checkpoint(s) for '{name}'")
                    else:
                        print(f"  ⚠️  No checkpoints found matching: {name}")
        else:
        ...
        # Sort by full directory name (timestamp + suffix)
        # This groups checkpoints with same timestamp together
        checkpoints.sort(key=lambda x: x.name)
        
        print(f"\n✓ Total: {len(checkpoints)} checkpoint(s) to visualize")
        return checkpoints
    ...
    def extract_checkpoint_metadata(self, checkpoint_dir: Path) -> CheckpointMetadata:
        ...
        # Extract suffix from checkpoint directory name
        # Pattern: YYYYMMDD_HHMMSS_adipose_sybreosin_{SUFFIX}_1024_finetune
        # or: YYYYMMDD_HHMMSS_adipose_sybreosin_1024_finetune (no suffix)
        suffix = ""
        suffix_match = re.search(r'\d{8}_\d{6}_adipose_sybreosin_(.+?)_1024_finetune', checkpoint_dir.name)
        if suffix_match:
...
class MetricsVisualizer:
    ...
    def __init__(self, output_dir: str = "../checkpoints/segmentation/model_comparison_plots"):
    ...
    def save_summary_csv(self, data: List[Tuple[CheckpointMetadata, EvaluationMetrics]],
        ...
        for metadata, metrics in data:
            row = {
                ...
                'suffix': metadata.suffix if metadata.suffix else '',
...
Examples:
  ...
  python visualize_checkpoint_metrics.py --checkpoints 20251024_150723 --clean-test --stain
    ...
    # Output options
    parser.add_argument('--output', type=str, default='../checkpoints/segmentation/model_comparison_plots',
                       help='Output directory for plots (default: ../checkpoints/segmentation/model_comparison_plots)')
