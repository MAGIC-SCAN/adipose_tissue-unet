...
def resolve_weights_path(weights_arg: str, use_ema: bool = False) -> Tuple[str, str]:
    """
    Resolve weights path and checkpoint directory.
    
    Args:
        weights_arg: Path to weights file or checkpoint directory
        use_ema: If True, prefer EMA weights over best weights
        
    Returns:
        Tuple of (weights_path, checkpoint_dir)
    """
    if not weights_arg:
        raise ValueError(
    ...
    if Path(weights_path).is_dir():
        ...
        # Prevent using checkpoints/ root directory - require specific checkpoint
        ckpt_path = Path(ckpt_dir)
        if ckpt_path.name in ["checkpoints", "segmentation", "classifier_runs"]:
            raise ValueError(
                f"❌ Cannot use root checkpoint directory: {ckpt_dir}\n"
                f"Please specify a specific checkpoint directory, e.g.:\n"
                f"  checkpoints/segmentation/20251104_152203_adipose_sybreosin_1024_finetune\n"
                f"  checkpoints/classifier_runs/classifier_20251111_164323"
            )
        
        weights_file = _find_best_weights_in_dir(Path(ckpt_dir), use_ema=use_ema)
        if weights_file is None:
            weights_type = "EMA" if use_ema else "best"
            raise FileNotFoundError(f"No {weights_type} weights files found in directory: {ckpt_dir}")
        weights_path = str(weights_file)
    else:
        # Weights file provided directly
        ckpt_dir = str(Path(weights_path).parent)
        
        # Validate that parent is not checkpoints/ root
        ckpt_path = Path(ckpt_dir)
        if ckpt_path.name in ["checkpoints", "segmentation", "classifier_runs"]:
            raise ValueError(
                f"❌ Weights file appears to be in root checkpoint directory: {ckpt_dir}\n"
                f"Weights should be in a timestamped checkpoint subdirectory.\n"
                f"Please move your weights to a proper checkpoint directory."
            )
    
    return weights_path, ckpt_dir


_WEIGHT_CANDIDATES_BEST = [
...
_WEIGHT_CANDIDATES_EMA = [
    "weights_ema.weights.h5",
    "ema_weights_phase2.weights.h5",
    "ema_weights.weights.h5",
]

def _find_best_weights_in_dir(ckpt_dir: Path, use_ema: bool = False) -> Optional[Path]:
    # Select candidate list based on use_ema flag
    candidates = _WEIGHT_CANDIDATES_EMA if use_ema else _WEIGHT_CANDIDATES_BEST
    ...
    # If EMA requested but not found, try best weights as fallback
    if use_ema:
        print("⚠️  EMA weights not found, falling back to best weights")
        for name in _WEIGHT_CANDIDATES_BEST:
            p = ckpt_dir / name
            if p.exists():
                return p
...
def _detect_deep_supervision(ckpt_dir: Path) -> bool:
    """Detect if checkpoint was trained with deep supervision
    
    Reads training_settings.log to check for use_deep_supervision flag.
    Falls back to False if file not found (v2 checkpoints).
    
    Args:
        ckpt_dir: Checkpoint directory path
        
    Returns:
        True if deep supervision was used, False otherwise
    """
    settings_file = ckpt_dir / "training_settings.log"
    if not settings_file.exists():
        return False  # v2 checkpoints don't have this file
    
    try:
        with open(settings_file, 'r') as f:
            content = f.read()
            # Check for deep supervision flag in settings
            if 'use_deep_supervision: True' in content:
                return True
            if 'deep_supervision: True' in content:
                return True
    except Exception as e:
        print(f"⚠️  Could not read training settings: {e}")
...
class AdiposeUNet:
    ...
    def build_model(self, init_nb: int = 44, dropout_rate: float = 0.3, use_deep_supervision: bool = False):
        """Build U-Net with exact same architecture as training
        
        Args:
            init_nb: Initial number of filters
            dropout_rate: Dropout rate
            use_deep_supervision: Build multi-output model (for train_adipose_unet_3.py checkpoints)
        """
        ...
        if use_deep_supervision:
            # Deep supervision: auxiliary outputs at up3 and up2 (matches train_adipose_unet_3.py)
    ...
    def predict_single(self, image: np.ndarray, mean: float, std: float) -> np.ndarray:
        ...
        # Extract main output if using deep supervision
        if self.use_deep_supervision:
            if isinstance(prediction, dict):
                return prediction['main_out'][0]
            elif isinstance(prediction, list):
                return prediction[0][0]  # First output is main_out
            else:
                raise ValueError(f"Unexpected prediction type with deep supervision: {type(prediction)}")
        else:
            return prediction[0]
...
def run_publication_evaluation(val_data_root: str, weights_path: str, output_dir: str,
    ...
    print("\nBuilding model...")
    use_deep_supervision = _detect_deep_supervision(checkpoint_dir)
    if use_deep_supervision:
        print("🔬 Detected deep supervision architecture (train_adipose_unet_3.py)")
    else:
        print("📐 Using standard architecture (train_adipose_unet_2.py)")
    
    # Load model with correct architecture
    model = AdiposeUNet()
    model.build_model(init_nb=44, dropout_rate=0.3, use_deep_supervision=use_deep_supervision)
...
def main():
    """Main execution function with simplified direct path arguments"""
...
Examples:
  ...
  python full_evaluation_enhanced.py \\
    ...
    --test-dataset /home/luci/adipose_tissue-unet/data/Meat_Luci_Tulane/_test/stain_normalized/clean_test
  
  # With TTA and sliding window
  python full_evaluation_enhanced.py \\
    --weights checkpoints/20251104_152203_adipose_sybreosin_1024_finetune \\
    --test-dataset /home/luci/adipose_tissue-unet/data/Meat_Luci_Tulane/_test/stain_normalized/clean_test \\
    --use-tta --tta-mode full \\
    --sliding-window --overlap 0.5
  ...
  python full_evaluation_enhanced.py \\
    --weights checkpoints/latest/weights_best.weights.h5 \\
    --test-dataset data/_test/original/human_test \\
    --output custom_evaluation_results
        """
    )
    
    # Required arguments
    parser.add_argument('--weights', type=str, required=True,
                       help='Path to trained model weights file or checkpoint directory')
    parser.add_argument('--test-dataset', type=str, required=True,
                       help='Path to test dataset directory (e.g., /path/to/stain_normalized/clean_test)')
    
    # Optional output path
    parser.add_argument('--output', type=str, default='',
                       help='Output directory for results (default: {checkpoint_dir}/evaluation/{dataset}_{source}_{enhancements})')
    
    # Weight selection
    parser.add_argument('--ema', action='store_true', default=False,
                       help='Use EMA weights (weights_ema.weights.h5) instead of best weights (default: False)')
    
    # Evaluation options
    parser.add_argument('--optimize-threshold', action='store_true',
                       help='Optimize threshold on test set (use with caution - typically for validation only)')
    ...
    # Resolve weights path and checkpoint directory
    weights_path, checkpoint_dir = resolve_weights_path(args.weights, use_ema=args.ema)
    ...
    if not test_dataset_path.is_dir():
        print(f"❌ Test dataset path must be a directory: {test_dataset_path}")
        return 1
    
    # Extract dataset name and data source from path
    dataset_name = test_dataset_path.name
    parent_dir_name = test_dataset_path.parent.name
    
    # Infer data source from parent directory name
    data_source_suffix = 'stain' if 'stain' in parent_dir_name.lower() else 'original'
    ...
    # EMA weights
    if args.ema:
        enhancement_suffixes.append("ema")
    ...
    if args.output:
        ...
        # Default: checkpoint_dir/evaluation/{dataset}_{source}_{enhancements}
        output_dir = Path(checkpoint_dir) / "evaluation" / output_folder_name
    ...
    print(f"Data Source:   {data_source_suffix}")
    ...
    if not test_dataset_path.exists():
        print(f"❌ Test dataset not found: {test_dataset_path}")
        if test_dataset_path.parent.exists():
            print(f"   Available in {test_dataset_path.parent}:")
            available = [d.name for d in test_dataset_path.parent.iterdir() if d.is_dir()]
            print(f"   {available}")
        return 1
    
    # Validate images/masks directories exist
    images_dir = test_dataset_path / "images"
    masks_dir = test_dataset_path / "masks"
    if not images_dir.exists() or not masks_dir.exists():
        print(f"❌ Dataset structure invalid. Expected:")
        print(f"   {test_dataset_path}/images/")
        print(f"   {test_dataset_path}/masks/")
        return 1
    
    print(f"📂 Dataset path: {test_dataset_path}")
    print(f"   Images: {len(list(images_dir.glob('*.jpg')))} files")
    print(f"   Masks: {len(list(masks_dir.glob('*.tif')))} files")
    ...
    print(f"  Output Dir:     {output_dir}")
    print(f"\n  Enhancement Options:")
    print(f"    TTA:                {args.use_tta}" + (f" (mode: {args.tta_mode})" if args.use_tta else ""))
    print(f"    Sliding Window:     {args.sliding_window}" + (f" (overlap: {args.overlap}, blend: {args.blend_mode})" if args.sliding_window else ""))
    print(f"    Boundary Refine:    {args.boundary_refine}" + (f" (kernel: {args.refine_kernel})" if args.boundary_refine else ""))
    print(f"    Adaptive Threshold: {args.adaptive_threshold}")
    
    print(f"\n  Evaluation Options:")
    print(f"    Threshold Optimization: {args.optimize_threshold}")
    print(f"    Save Visualizations:    {not args.no_visualizations}" + (f" (n={args.n_vis_samples})" if not args.no_visualizations else ""))
    print(f"    Save Overlays:          {args.save_overlays}" + (f" (+{args.n_positive}/-{args.n_negative})" if args.save_overlays else ""))
    print(f"{'='*80}\n")
    
    # Run publication-quality evaluation
    try:
        results = run_publication_evaluation(
            val_data_root=str(test_dataset_path),
        ...
        print(f"{'='*80}")
        print(f"  Dice Score:      {results.dice_score:.4f} (95% CI: [{results.dice_ci[0]:.4f}, {results.dice_ci[1]:.4f}])")
        print(f"  Jaccard (IoU):   {results.jaccard_index:.4f} (95% CI: [{results.jaccard_ci[0]:.4f}, {results.jaccard_ci[1]:.4f}])")
        print(f"  Precision:       {results.precision:.4f} (95% CI: [{results.precision_ci[0]:.4f}, {results.precision_ci[1]:.4f}])")
        print(f"  Sensitivity:     {results.sensitivity:.4f} (95% CI: [{results.sensitivity_ci[0]:.4f}, {results.sensitivity_ci[1]:.4f}])")
        print(f"  Specificity:     {results.specificity:.4f} (95% CI: [{results.specificity_ci[0]:.4f}, {results.specificity_ci[1]:.4f}])")
        print(f"  Optimal Thresh:  {results.optimal_threshold:.3f}")
        print(f"  Slides:          {results.n_slides}")
        ...
        print(f"Error: {e}")
        print("\nCommon causes:")
        print("1. Missing or corrupt image/mask files")
        print("2. Invalid weights file or architecture mismatch")
        print("3. Incompatible normalization stats")
        print("4. Insufficient GPU memory for batch processing")
        print("5. Dataset structure mismatch (expected images/ and masks/ subdirectories)")
        print(f"{'='*80}\n")
        import traceback
        traceback.print_exc()
        return 1
